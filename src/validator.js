const validator = {
  ...
  const arrayStringNumber = Array.from(cardNumber);// transformo el string cardNumber en un array de elementos string ['1','0',....]
  const toNumbers = arrayStringNumber.map(i=>Number(i)); // transformo los elementos string del array en numeros
  //algoritmo de luhn
  ...
  const digitsLessThan10 = parElementsx2.filter(number=>number<10); // filtro los numeros < 10 del array
  ...
  if (sumTotalElements%10 === 0){ //en caso el residuo de esa suma sea igual a 0 entonces la tarjeta es valida
    return 'es válida';
    } else {
    return 'no es válida';
    }
